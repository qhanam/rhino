...
public abstract class AstNode extends Node
	implements Comparable<AstNode>, ClassifiedASTNode, Cloneable {
    ...
    /**
     * The change type from AST differencing. INSERT/DELETE: This node or its parent
     * was inserted or removed. UPDATED: The contents of this node or one of its
     * descendants was changed.
     * 
     * @author qhanam
     **/
    ...
    /**
     * The change type from AST differencing without propagation. INSERT/DELETE:
     * This node and its unlabeled descendants was inserted or removed. UPDATED:
     * This node's value was changed.
     * 
     * @author qhanam
     **/
    ...
    /**
     * The version of the node relative to the commit. Can be source or destination
     * node.
     * 
     * @author qhanam
     */
    ...
    /**
     * The relations for which this AST node is a criterion.
     */
    protected Map<String, Integer> criteria;

    /**
     * The list of relations and criteria for which this AST node has a dependency.
     */
    protected Map<String, Set<Integer>> dependencies;

    /**
     * Because the analysis is partial, the origin of some values cannot be tracked.
     * In these cases, we provide dummy value declarations, which are expressions
     * which may provide new values. This field tracks which {@code AstNodes}
     * provided dummy values during the analysis.
     * 
     ...
     * @author qhanam
     */
    public void addCriterion(String relation, Integer id) {
	...
	for (Entry<String, Integer> entry : this.criteria.entrySet()) {
	    ...
	    criteriaJson.addProperty(relation, address);
	}
	return criteriaJson;
    }
	...
	for (Entry<String, Set<Integer>> entry : this.dependencies.entrySet()) {
	    String relation = entry.getKey();
	    Set<Integer> addresses = entry.getValue();
	    JsonArray dependencyAddresses = new JsonArray();
	    for (Integer address : addresses) {
		dependencyAddresses.add(address);
	    }
	    dependencyJson.add(relation, dependencyAddresses);
	}
    ...
    /**
     * @param moved
     *            {@code true} if this node has a 'moved' edit operation applied by
     *            AST differencing.
     * @author qhanam
     */
    @Override
    ...
    /**
     * @return {@code true} if this node has a 'moved' edit operation applied by AST
     *         differencing.
     * @author qhanam
     */
    ...
    /**
     * @param changeType
     *            The non-propagated change applied to this node from AST
     *            differencing.
     * @author qhanam
     */
    @Override
    ...
    /**
     * Clones the AstNode. This covers the following AstNode types (i.e., anything
     * that doesn't have children): - Name - Break - Continue - Comment -
     * EmptyExpression - EmptyStatement - KeywordLiteral - Label - NumberLiteral -
	...
	if (!(node instanceof AstNode))
	    throw new InvalidClassException("The ClassifiedASTNode is not an AstNode.");
	...
	/**
	 * Sorts nodes by (relative) start position. The start positions are relative to
	 * their parent, so this comparator is only meaningful for comparing siblings.
	 */
	@Override
    ...
    /**
     * Returns the absolute document position of the node. Computes it by adding the
     * node's relative position to the relative positions of all its parents.
     */
    ...
    /**
     * Make this node's position relative to a parent. Typically only used by the
     * parser when constructing the node.
     * 
     * @param parentPosition
     *            the absolute parent position; the current node position is assumed
     *            to be absolute and is decremented by parentPosition.
     */
    ...
    /**
     * Sets the node parent. This method automatically adjusts the current node's
     * start position to be relative to the new parent.
     * 
     * @param parent
     ...
     * relative to this node. Sets the length of this node to include the new child.
     * 
     * @param kid
     *            the child
     ...
     * {@code null} children that are expected to be non-{@code null} when no errors
     * are present. In this situation, the behavior of the {@code toSource} method
     * is undefined: {@code toSource} implementations may assume that the AST node
     * is error-free, since it is intended to be invoked only at runtime after a
     * successful parse.
     * <p>
     *
     * @param depth
     *            the current recursion depth, typically beginning at 0 when called
     *            on the root node.
     */
     ...
     * children. The subclass also decides (and should document) which child nodes
     * are not passed to the {@code NodeVisitor}. For instance, nodes representing
     * keywords like {@code each} or {@code in} may not be passed to the visitor
     * object. The visitor can simply query the current node for these children if
     * desired.
     * <p>
     *
     * Generally speaking, the order will be deterministic; the order is whatever
     * order is decided by each child node. Normally child nodes will try to visit
    ...
    /**
     * Returns the innermost enclosing {@link Scope} node, or {@code null} if we're
     * not nested in a scope. Begins the search with this node's parent. Note that
     * this is not the same as the defining scope for a {@link Name}.
     *
    ...
    /**
     * Permits AST nodes to be sorted based on start position and length. This makes
     * it easy to sort Comment and Error nodes into a set of other AST nodes: just
     * put them all into a {@link java.util.SortedSet}, for instance.
     * 
     * @param other
     *            another node
     * @return -1 if this node's start position is less than {@code other}'s start
     *         position. If tied, -1 if this node's length is less than
     *         {@code other}'s length. If the lengths are equal, sorts abitrarily on
     *         hashcode unless the nodes are the same per {@link #equals}.
     */
    @Override
    ...
    /**
     * Returns the depth of this node. The root is depth 0, its children are depth
     * 1, and so on.
     * 
     ...
     * 
     * @return a very verbose indented printout of the tree. The format of each line
     *         is: abs-pos name position length [identifier]
     */
